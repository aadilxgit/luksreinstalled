...
for needle in 'cryptsetup luksFormat' 'cryptroot UUID=%s none luks' 'grub-install' 'reinstall-done' 'debootstrap ' 'luksUUID' 'mount --bind /proc /mnt/proc' 'init-premount' 'exec tail -n +3 "\$0"' 'Debian LUKS reinstall (retry)'; do
